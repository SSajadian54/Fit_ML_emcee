...
prt=([1.12144159e+00,   3.65038760e-01,   3.44971539e-01  , 3.29139600e-02,6.16781148e+00   ,7.24754419e+01 ,  6.83584800e+03 , -9.57372677e-03, 1.35197350e-01  ,-3.77845230e-01 ,  1.60454048e+00   ,4.69653592e-02,9.41215237e+00  , 6.62235300e+00,   4.11754921e+00,   3.01145746e+00,1.01571164e+00 ,  1.17367258e+00 ,  2.73154066e+00 ,  2.33234330e+00])



Epci=np.array([0.05,0.05,0.03,float(2.5*RA),0.5,3.5,7.0,0.08,0.08,2.5,2.5,2.5,3.5,2.5,2.0,2.0,0.6,0.6,1.3,1.3])
...
def prior(p): 
    ...
    float(p[13])<0.0 or float(p[14])<0.0 or float(p[15])<0.0 or float(p[16])<0.0 or float(p[17])<0.0 or float(p[18])<0.0 or 
    float(p[19])<0.0 or float(p[12])==0.0 or float(p[14])==0.0 or float(p[16])==0.0  or float(p[18])==0.0):
        tet=int(0) 
...
def lnlike2(p,timed,magnio,errom):
    ...
    fsc=float(pr[16]*Fsc);    fbgc=float(pr[17]*Fbgc); 
    fss=float(pr[18]*Fss);    fbgs=float(pr[19]*Fbgs); 
...
###============================= Main program ===================================
array1=np.zeros((NUM1,3)); array2=np.zeros((NUM2,3)); array3=np.zeros((NUM3,3)); array4=np.zeros((NUM4,3))
sda=np.zeros((numt,3))
...
sda=np.concatenate((array1,array2,array3,array4),axis=0)
...
print "chi2: ", float(chi2(prt,timed,magnio,errom))
...
fil=open("./files/finalODCS141050_0504.dat","w")
fil.close()
for pos, prob1, state in sampler.sample(p0,iterations=Nstep,storechain=False):
    fil = open("./files/finalODCS141050_0504.dat","a")
    for i in range(Nwalkers):   
        if(np.isfinite(prob1[i])): 
            ssa=np.concatenate((pos[i,:].reshape((1,NDIM)),prob1[i].reshape((1,1))),axis=1)
            np.savetxt(fil,ssa,fmt="%.8f %.8f %.8e %.8f %.10e %.8f %.8f %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e %.8e  %.8f")
    fil.close()
print("***************** END OF MCMC *********************** ")
...
array_last=np.loadtxt("files/finalODCS141050_0504.dat") 
samples1 = array_last[:,:NDIM].reshape((-1,NDIM))


print("******* The Best Fitted parameters  ********")
save2=open("files/bestfitted1050_0504.txt","w")
